...
- `render_dashboard.py` — renders a token-focused SELF-CONTAINED HTML dashboard (inline CSS/JS/SVG, NO network/CDN): **Your usage limits** gauges (Chat & Claude Code / Cowork credit as a % of limits set in `config/usage-limits.json` — local-used ÷ your-limit, NOT server quota; air-gapped) + **Token usage · who spent what** bars (by conversation/project/skill, each with % share) + **Usage over time** daily curve, light/dark. Reads `worktemp/token-usage.json` + `config/usage-limits.json` (falls back to the template = unset). Output → `worktemp/dashboard.html` (private). The air-gapped alternative to a cloud dashboard (RL1/RL4).
